                ...
                if now - last_buttons_time >= 0.5:
                    if self.cip.buttons_pressed:
                        with self.cip.buttons_lock:
                            # Performance optimization: use .items() to avoid overhead of repeated dictionary lookups
                ...
                if update_request_type == 0x00:
                    ...
                    with self.join_lock:
                        for sigtype, joins in self.join["out"].items():
                            # Performance optimization: use .items() to bypass deep key lookup and speed up end-of-query
                            for j, values in joins.items():
                                self.set(sigtype, j, values[0])
