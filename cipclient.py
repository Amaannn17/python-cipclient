                ...
                if now - last_buttons_time >= 0.5:
                    if self.cip.buttons_pressed:
                        with self.cip.buttons_lock:
                            # ⚡ Bolt: Use .items() to avoid dict lookup in hot polling loop
                ...
                if update_request_type == 0x00:
                    ...
                    with self.join_lock:
                        for sigtype, joins in self.join["out"].items():
                            # ⚡ Bolt: Use .items() to avoid dict lookup during end-of-query processing
                            for j, val in joins.items():
                                self.set(sigtype, j, val[0])
