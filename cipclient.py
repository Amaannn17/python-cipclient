                ...
                if now - last_buttons_time >= 0.5:
                    if self.cip.buttons_pressed:
                        with self.cip.buttons_lock:
                            # Performance optimization: use .items() to iterate
                            # over key-value pairs rather than keys alone to avoid
                            # an extra dictionary lookup on every button press loop iteration.
                            for join, packet in self.cip.buttons_pressed.items():
                                try:
                                    if self.cip.join["out"]["d"][join][0] == 1:
                                        self.cip.tx_queue.put(packet)
                                except KeyError:
                                    pass
                    last_buttons_time = now
                ...
                if update_request_type == 0x00:
                    ...
                    with self.join_lock:
                        for sigtype, joins in self.join["out"].items():
                            # Performance optimization: use .items() to iterate
                            # over key-value pairs rather than keys alone to avoid
                            # an extra dictionary lookup when processing outgoing join state updates.
