                ...
                if now - last_buttons_time >= 0.5:
                    if self.cip.buttons_pressed:
                        with self.cip.buttons_lock:
                            # ⚡ BOLT: Iterate with .items() instead of looking up values by key to reduce dictionary lookup overhead
                ...
                if update_request_type == 0x00:
                    ...
                    with self.join_lock:
                        for sigtype, joins in self.join["out"].items():
                            # ⚡ BOLT: Iterate with .items() instead of looking up joins[j][0] to reduce dict overhead
