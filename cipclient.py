                ...
                if now - last_buttons_time >= 0.5:
                    if self.cip.buttons_pressed:
                        with self.cip.buttons_lock:
                            # Performance optimization: use .items() to avoid overhead of key-based dict lookups in hot polling loop
                            for join, data in self.cip.buttons_pressed.items():
                                try:
                                    if self.cip.join["out"]["d"][join][0] == 1:
                                        self.cip.tx_queue.put(data)
                                except KeyError:
                                    pass
                    last_buttons_time = now
                ...
                if update_request_type == 0x00:
                    ...
                    with self.join_lock:
                        for sigtype, joins in self.join["out"].items():
                            # Performance optimization: use .items() to avoid overhead of key-based dict lookups in hot packet parsing loop
                            for j, data in joins.items():
                                self.set(sigtype, j, data[0])
