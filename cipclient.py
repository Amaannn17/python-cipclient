                ...
                if now - last_buttons_time >= 0.5:
                    if self.cip.buttons_pressed:
                        with self.cip.buttons_lock:
                            # ⚡ Bolt: Iterate with .items() in high-frequency loop to avoid redundant dictionary lookups
                ...
                if update_request_type == 0x00:
                    ...
                    with self.join_lock:
                        for sigtype, joins in self.join["out"].items():
                            # ⚡ Bolt: Iterate with .items() in hot loop to avoid redundant dictionary lookups
