        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                if not self._stop_event.is_set():
                    # ⚡ Bolt: Use stop_event.wait() instead of time.sleep() for <1ms shutdown latency
                ...
                while (
                    ...
                    # ⚡ Bolt: Use stop_event.wait() instead of time.sleep() for responsive thread termination
                    self._stop_event.wait(1)
                if not self._stop_event.is_set():
                    self.cip.connected = False
                    self.cip.socket.close()
                    _logger.debug(f"lost connection to {self.cip.host}:{self.cip.port}")
                    # ⚡ Bolt: Use stop_event.wait() instead of time.sleep() to avoid blocking thread shutdown
                    self._stop_event.wait(10)
                else:
