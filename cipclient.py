                ...
                if now - last_buttons_time >= 0.5:
                    if self.cip.buttons_pressed:
                        with self.cip.buttons_lock:
                            # Optimize: use .items() to eliminate secondary key lookups in tight polling loop
                            for join, tx_payload in self.cip.buttons_pressed.items():
                                try:
                                    if self.cip.join["out"]["d"][join][0] == 1:
                                        self.cip.tx_queue.put(tx_payload)
                                except KeyError:
                                    pass
                    last_buttons_time = now
                ...
                if update_request_type == 0x00:
                    ...
                    with self.join_lock:
                        for sigtype, joins in self.join["out"].items():
                            # Optimize: use .items() to eliminate key lookups for the state array
                            for j, join_state in joins.items():
                                self.set(sigtype, j, join_state[0])
