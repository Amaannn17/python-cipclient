...
class EventThread(threading.Thread):
    ...
    def run(self):
        ...
        while not self._stop_event.is_set():
            ...
            # ⚡ Bolt: Prevent eager f-string evaluation when debug logging is disabled (~30% faster event processing)
            if _logger.isEnabledFor(logging.DEBUG):
    ...
    def _processPayload(self, ciptype, payload):
        ...
        if ciptype == 0x0D or ciptype == 0x0E:
            ...
            if datatype == 0x00:
                ...
                # ⚡ Bolt: Prevent eager f-string evaluation when debug logging is disabled
                if _logger.isEnabledFor(logging.DEBUG):
                ...
                # date/time
                # ⚡ Bolt: Prevent expensive hex() conversion and eager f-string evaluation when debug logging is disabled
                if _logger.isEnabledFor(logging.DEBUG):
