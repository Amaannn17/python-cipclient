                ...
                if now - last_buttons_time >= 0.5:
                    if self.cip.buttons_pressed:
                        with self.cip.buttons_lock:
                            # ⚡ Bolt: Iterate over .items() to avoid overhead of repeated dictionary lookups in this hot loop
                ...
                if update_request_type == 0x00:
                    ...
                    with self.join_lock:
                        for sigtype, joins in self.join["out"].items():
                            # ⚡ Bolt: Iterate over .items() to avoid dictionary key lookup overhead in this end-of-query path
