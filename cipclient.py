                ...
                if now - last_buttons_time >= 0.5:
                    if self.cip.buttons_pressed:
                        with self.cip.buttons_lock:
                            # Optimize: use .items() to avoid dictionary lookups in hot polling loop
                            for join, tx_value in self.cip.buttons_pressed.items():
                                try:
                                    if self.cip.join["out"]["d"][join][0] == 1:
                                        self.cip.tx_queue.put(tx_value)
                                except KeyError:
                                    pass
                    last_buttons_time = now
                ...
                if update_request_type == 0x00:
                    ...
                    with self.join_lock:
                        for sigtype, joins in self.join["out"].items():
                            # Optimize: use .items() to avoid dictionary lookup in hot path
                            for j, callback_list in joins.items():
                                self.set(sigtype, j, callback_list[0])
